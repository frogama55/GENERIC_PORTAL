...
- 2カラム化（掲示＝左／スケジュール＝右）を試作したが，依頼者判断で**元のレイアウトに戻した**
  （`#mainWrapBottomPortal` の flex 化を撤去）．5件表示制限（`li.ui-datalist-item:nth-child(n+6)`）
  は維持している．
...
- 実装（attendance.js）：出欠セルの中身を `div.cit-att-cell` で包み「日付→マーク」に並べ替える．
  ※ 日付への曜日付与は一度実装したが，依頼者判断で**削除**（曜日は左端の「曜日時限」で分かるため）．
...
- どのページでも「戻る」を押すと掲示一覧に飛ばされる問題への対策として `navfix.js` を試作したが，
  対象ページがサーバー側にビュー状態(JSF ViewState)を持っており，同一URLをGETで取り直すと
  **HTTP 500** になるため実用にならなかった．依頼者判断で機能・設定項目とも実装ごと削除した
  （manifest・settings.html/js からも除去）．経緯の詳細は削除前のgit履歴を参照．
